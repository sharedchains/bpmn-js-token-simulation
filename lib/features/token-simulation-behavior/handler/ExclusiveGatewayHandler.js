...
function ExclusiveGatewayHandler(eventBus, animation, elementRegistry, exclusiveGatewaySettings) {
...
ExclusiveGatewayHandler.prototype.generate = function(context) {
  ...
  this._animation.createAnimation(sequenceFlow, processInstanceId, function() {
    ...
    self._exclusiveGatewaySettings.resetSequenceFlowToDark(element);
  });
};

ExclusiveGatewayHandler.$inject = [ 'eventBus', 'animation', 'elementRegistry', 'exclusiveGatewaySettings' ];

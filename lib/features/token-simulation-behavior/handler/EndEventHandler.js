...
EndEventHandler.prototype.consume = function(context) {
  ...
  var isTerminate = isTypedEvent(getBusinessObject(element), 'bpmn:TerminateEventDefinition'),
      ...
      isParticipant = is(element.parent, 'bpmn:Participant');
  ...
  if ((isFinished || isTerminate) && (isSubProcessChild || isParticipant)) {
    ...
    this._eventBus.fire(GENERATE_TOKEN_EVENT, {
      element: isSubProcessChild ? element.parent : element,
      processInstanceId: isSubProcessChild ? processInstance.parentProcessInstanceId : processInstanceId
    });
  }
...
/**
 * End event generates only in case of MessageEndEvent.
 */
